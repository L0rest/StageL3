...
concentration = {1: [10**(t/10) for t in range(-60, 0)], 2: [1e-8],
    3: [1e-8, 5e-8, 1e-7, 5e-7, 1e-6, 5e-6, 1e-5, 5e-5, 1e-4, 3e-4, 5e-4, 7e-4, 9e-4, 1e-3, 3e-3, 5e-3, 7e-3, 9e-3, 1e-2, 3e-2, 5e-2, 7e-2, 9e-2, 1e-1],
    4: [1e-13, 5e-12, 1e-12, 5e-11, 1e-11, 5e-10, 1e-9, 5e-9, 1e-8, 5e-8, 1e-7, 5e-7, 1e-6, 5e-6, 1e-5, 5e-5, 1e-4, 5e-4, 1e-3, 5e-3, 1e-2, 5e-2, 1e-1]}
                ...
                for j in range(len(E)):
                    if (nbY[j] == 1) and (nbV[j] == 0) and (j != i):
                        if E[j]<E[i]:
                            ...
                            E_monosol= E[i]
                            indice_monosol= i
                print(f"Le monosoluté le plus stable a été trouvé: en position {indice_monosol-1}, nom : {deftype[indice_monosol]}")
...
def calculpotentielv1(nY, nV, Eb, dtype, EFv, fileout, muMIN = -12, muMAX = -3, atom = "H"):
    ...
    with open(fileout, 'w', encoding='utf8') as output:
        ...
        while muY < muMAX:
            ...
            output.writelines(("{:15.4e} "*(len(clustersEf)+1)).format(muY, *[clustersEf[i] for i in range(len(clustersEf))]))
            output.writelines("\n")
            muY += 0.01
        ...
        for Ytot in conc: # Boucle sur les concentrations
            ...
            while T < Tf: # Boucle sur les T
                ...
                try:
                    ...
                    output.writelines(("{:15.5e} " * (len(clusters) + 1)).format(T, *[clusternum[i] for i in range(len(clusters))]))
        ...
        for T in Temp: # Boucle sur les températures
            ...
            for Ytot in concentration[4]: # Boucle sur les concentrations (valeurs linéaires)
                ...
                try:
                    ...
                    for x in range(len(values)):
                        clusternum = [item.subs(symvar[x], values[x]) for item in clusternum]
                    output.writelines(("{:15.5e} " * (len(clusters) + 1)).format(Ytot, *[clusternum[i] for i in range(len(clusters))]))
                    # output.writelines(("{:15.4e} " * (len(clusters) + 1)).format(T, *[clusternum[i] for i in range(len(clusters))]))
                    output.writelines("\n")
                    amas.append(clusters)
                    amasnum.append(clusternum)
...
def trace(): #Affichage des fonctions
    ...
    ## &Kevin-debut&
    if isoT_flag:
        calcul_grand_canonique_isoth(nY, nV, g, Eb, dtype, charge, mini, maxi, pas, EFv, nameoutput, canonique, atome, oxyde_flag)
        tracer_isoth()
    else:
        calcul_grand_canonique(nY, nV, g, Eb, dtype, charge, mini, maxi, pas, conc, EFv, nameoutput, canonique, atome, oxyde_flag)
        tracer()
    ## &Kevin-fin&
def tracer(): # Pour afficher le graphe sélectionné
    global select
    if figs: # Si les graphes ont été tracés...
        canvas1 = FigureCanvasTkAgg(figs[select % len(figs)][0], master = my_canvas)
        canvas1.get_tk_widget().place(x = 0, y = 0)
        tot_label.config(text = "Concentration sélectionnée: " + str(figs[select % len(figs)][2]))
    else:
        tkt.messagebox.showwarning("Graphe", "Les graphes n'ont pas pu être tracés!")
...
def tracer_isoth(): # Pour afficher le graphe sélectionné
    ...
    if figs: # Si les graphes ont été tracés...
        canvas1 = FigureCanvasTkAgg(figs[select % len(figs)][0], master = my_canvas)
        canvas1.get_tk_widget().place(x = 0, y = 0)
        tot_label.config(text = "Température sélectionnée: " + str(figs[select % len(figs)][2]))
    else:
        tkt.messagebox.showwarning("Graphe", "Les graphes n'ont pas pu être tracés!")
## &Kevin-fin&
...
def recuperation(): # Fonction récupérant les valeurs à partir des données saisies
    ...
    if v1 > v2:
        v2, v1 = v1, v2
        tkt.messagebox.showwarning("Températures", "La température initiale doit être plus élevée que la température initiale !")
    ...
    ## &Kevin-debut&
    if v7 <= 0:
        tkt.messagebox.showerror("Nombre atomes par maille", "Le nombre d'atomes par maille doit être strictement positif !")
    ...
    if vb == "GCy: lacunaire uniquement":
        ...
        tkt.messagebox.showwarning("Canonique", "L'ensemble canonique sélectionné est incorrect; le mode lacunaire uniquement est sélectionné par défaut.")
    ...
    ## &Kevin-fin&
        ...
        tkt.messagebox.showwarning("Atomes", "Le cristal sélectionné est incorrect; le Béryllium est sélectionné par défaut.")
    ...
    if vd == "Exponentiel borné":
        ...
        tkt.messagebox.showerror("Concentrations", "Le mode de concentration sélectionné est incorrect; les valeurs linéaires sont sélectionnées par défaut.")
    ...
    if ve == "Oxydes":
        ...
        ve = False
        tkt.messagebox.showwarning("Charges", "L'option sélectionnée est incorrecte; les charges sont par défaut désactivées.")
    ...
    if vf == "DFT":
        ...
        vf = False
        tkt.messagebox.showwarning("énergies de liaison", "L'option sélectionnée est incorrecte; les énergies de liaison sont par défaut désactivées.")
    ## &Kevin-debut&
    if vg == "Isotherme":
        vg = True
        ...
        vg = True
        tkt.messagebox.showwarning("Mode de graphiques", "L'option sélectionnée est incorrecte; les graphes sont par défaut isothermes.")
    ## &Kevin-fin&
    ## &Kevin-debut&
...
cadre0 = tkt.LabelFrame (fenetre, text = "THERMO", width = 750, height = 500)
texte0 = tkt.Label (cadre0, text = "Version: 0.3.1 du 14/12/2023 par Kevin Gautier \n Versions précédentes: 0.2 du 06/05/2023 par Gabriel Faraut \n 0.1 du 25/06/2021 par Damien Connétable \n E_mail: damien.connetable@ensiacet.fr \n CNRS CIRIMAT")
texte0.pack()
cadre0.pack(fill = "both", expand = "yes")

my_canvas = tkt.Canvas(cadre0, width = 750, height = 500)
...
temperature_initiale = tkt.Entry (cadre1, width = 5)
titre_initiale = tkt.Label (cadre1, text = "Entrez ici la température initiale :")
temperature_finale = tkt.Entry (cadre1, width = 5)
titre_finale = tkt.Label (cadre1, text = "Entrez la température maximale :")
temperature_pas = tkt.Entry (cadre1, width = 5)
titre_pas = tkt.Label (cadre1, text = "Entrez le pas de calcul :")
dim_x = tkt.Entry (cadre1, width = 2)
dim_y = tkt.Entry (cadre1, width = 2)
dim_z = tkt.Entry (cadre1, width = 2)
titre_dim = tkt.Label (cadre1, text = "Entrez les dimensions x, y et z du cristal :")
## &Kevin-debut&
nb_atom_maille = tkt.Entry (cadre1, width = 2)
titre_atom = tkt.Label (cadre1, text = "Entrez le nombre d'atomes par maille :")
## &Kevin-fin&

cadre2 = tkt.LabelFrame(fenetre, text = "Sélection des atomes", padx = 20, pady = 20)
...
atomes_y = ['H: Dihydrogène', 'C: Carbone allotrope diamant', 'N: Diazote', 'O: Dioxygène', 'Dihydrogène, version de débugage']
...
my_canvas.create_window((0, 0), window = cadre0, anchor = "nw")

etape1 = tkt.Button (cadre2, text = "Vérifier les données saisies", command = recuperation)
...
cadre0.pack()
cadre1.pack()
titre_initiale.grid(row = 0, column = 0)
temperature_initiale.grid(row = 0, column = 1)
titre_finale.grid(row = 0, column = 2)
temperature_finale.grid(row = 0, column = 3)
titre_pas.grid(row = 1, column = 0)
temperature_pas.grid(row = 1, column = 1)
titre_dim.grid(row = 1, column = 2)
dim_x.grid(row = 1, column = 4)
dim_y.grid(row = 1, column = 5)
dim_z.grid(row = 1, column = 6)
## &Kevin-debut&
titre_atom.grid(row = 1, column = 7)
nb_atom_maille.grid(row = 1, column = 8)
## &Kevin-fin&
